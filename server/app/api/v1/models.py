...
def list_models(db: DbDep, user: CurrentUserDep) -> list[ModelResponse]:
    rows = model_service.list_models(db, user_id=str(user.id))
    return [model_to_response(row) for row in rows]


@router.get("/status", response_model=ModelStatusResponse)
def models_status(db: DbDep, user: CurrentUserDep) -> ModelStatusResponse:
    payload = model_service.get_models_status(db, user_id=str(user.id))
...
def test_stored_model_connection(
    model_id: int, db: DbDep, user: CurrentUserDep
) -> ModelTestConnectionResponse:
    error = model_service.test_stored_model_connection(db, model_id, user_id=str(user.id))
    if error:
        return ModelTestConnectionResponse(ok=False, message=error)
    return ModelTestConnectionResponse(ok=True, message="连接成功")
...
def create_model(body: ModelCreateRequest, db: DbDep, user: CurrentUserDep) -> ModelResponse:
...
def update_model(
    model_id: int, body: ModelUpdateRequest, db: DbDep, user: CurrentUserDep
) -> ModelResponse:
...
def delete_model(model_id: int, db: DbDep, user: CurrentUserDep) -> Response:
    model_service.delete_model(db, model_id, user_id=str(user.id))

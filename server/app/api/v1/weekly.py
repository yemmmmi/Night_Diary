...
def generate_weekly(db: DbDep, container: ContainerDep) -> WeeklyReportResponse:
    row = weekly_service.generate_weekly_report(db, container)
    return weekly_to_response(row)


@router.post("/regenerate", response_model=WeeklyReportResponse)
def regenerate_weekly(db: DbDep, container: ContainerDep) -> WeeklyReportResponse:
    row = weekly_service.regenerate_weekly_report(db, container)
    return weekly_to_response(row)
...
def list_weekly(
    ...
    rows = weekly_service.list_reports(db, skip=skip, limit=limit)
    return [weekly_to_response(row) for row in rows]


@router.get("/latest", response_model=WeeklyReportResponse)
def latest_weekly(db: DbDep) -> WeeklyReportResponse:
    row = weekly_service.get_latest_report(db)
    return weekly_to_response(row)

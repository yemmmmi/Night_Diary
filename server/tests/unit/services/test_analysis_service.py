...
from app.shared.errors import AnalysisNotFoundError, DiaryAlreadyExistsError
...
def test_delete_analysis_for_diary_clears_reply(db_session) -> None:
    ...
    assert db_session.query(AnalysisRow).filter_by(diary_id=entry.id).count() == 0

    with pytest.raises(AnalysisNotFoundError):
        analysis_service.get_analysis_by_id(db_session, 999999, user_id="default")

...
def test_compress_keeps_query_relevant_entries() -> None:
    ...
    result = compressor.compress(
        "今天又失眠了，脑子里全是工作的事。",
        episodic=[
            {"event": "连续三天失眠", "content": "连续三天失眠，凌晨两点才睡着，白天精神很差"},
            {"event": "周末去爬山", "content": "周末去爬山，天气很好，心情不错，拍了好多照片"},
        ],
    )
    assert "失眠" in result
...
def test_compress_respects_token_budget() -> None:
    long_episodic = [
        {"event": f"重要事件{i}", "content": "失眠焦虑压力" * 40}
        for i in range(10)
    ]
...
def test_compress_skips_low_density_entries() -> None:
    ...
    result = compressor.compress(
        ...
        episodic=[
            {"event": "早安", "content": "早安"},
            {"event": "连续加班导致身心疲惫", "content": "连续加班导致身心疲惫，需要休息"},
        ],
    )
    assert "加班" in result
...
def test_compress_summarizes_long_entries_without_llm() -> None:
    ...
    result = compressor.compress(
        "回顾今天。",
        episodic=[{"event": "长日记", "content": long_text}],
    )
...
def test_prepare_compressed_history_from_state() -> None:
    state = {
        "diary_content": "我又失眠了。",
        "episodic_context": [{"event": "上周也失眠", "content": "上周也失眠，整晚翻来覆去睡不着"}],
    }

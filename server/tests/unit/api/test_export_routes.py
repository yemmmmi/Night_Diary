...
def test_export_all_reachable(authed_client: TestClient) -> None:
    """GET /api/v1/export/all returns 200 with valid export structure."""
    response = authed_client.get("/api/v1/export/all")
...
def test_import_json_reachable(authed_client: TestClient) -> None:
    """POST /api/v1/import/json accepts exported data and returns summary."""
    # First export the current (empty) state
    exported = authed_client.get("/api/v1/export/all").json()

    # Import it back
    response = authed_client.post("/api/v1/import/json", json={"data": exported})
    assert response.status_code == 200
    body = response.json()
